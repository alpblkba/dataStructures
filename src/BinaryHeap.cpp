...
void BinaryHeap::percolateDown(int hole) {
	...
	/*int left = heap[hole * 2];
	int right = heap[hole * 2 + 1];
	if (left > heap[hole] && right > heap[hole]) {
		return;
	}
	if (hole*2 + 1 < size && left < right) {
		if (heap[hole] > left) {
			swap(hole, hole * 2);
			percolateDown(hole * 2);
		}
	}
	else if (hole * 2 + 1 < size && left <= heap[hole]) {
		swap(hole, hole*2);
		percolateDown(hole*2);
	}
	else if (hole * 2 + 1 < size && right <= heap[hole]) {
		swap(hole, hole*2 +1);
		percolateDown(hole*2 + 1);
	}
	*/
